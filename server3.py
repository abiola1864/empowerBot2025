...
def handle_text_message(phone_number, message_body, user, conn):
    ...
    if random.random() < 0.05:  # 5% chance to show the reminder
...
def handle_records_command(phone_number, user, conn):
    send_message(phone_number, f"Welcome to Record Keeping, {user['name']}! Please upload your business record as an image or PDF.")
    conn.execute('UPDATE users SET state = ? WHERE phone_number = ?', ('records', phone_number))
    conn.commit()
    # Optionally, you can add a reminder about switching modes
    #send_message(phone_number, "Remember, you can type 'quiz' at any time to switch to quiz mode.")
...
def handle_button_response(phone_number, button_id, button_text, user, conn):
    ...
    try:
        if button_id == "records" or button_text.lower() == "switch to records":
            handle_records_command(phone_number, user, conn)
        elif button_id == "quiz" or button_text.lower() == "take another quiz":
            handle_quiz_command(phone_number, user, conn)
        else:
        ...
        log_image_event(traceback.format_exc())
        send_message(phone_number, "An error occurred. Please try again or type 'records' or 'quiz' to switch.")
...
def handle_quiz_response(phone_number, response, user, conn):
    ...
    if question_index < len(QUIZ_QUESTIONS):
        ...
        is_correct = response.lower() == correct_answer
...
def scoreboard():
    ...
    try:
        ...
        return render_template('scoreboard.html', 
                               scores=user_scores,  # Pass as Python object, not JSON string
